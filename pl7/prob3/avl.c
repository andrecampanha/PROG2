...
no_avl* avl_insere_impl(no_avl *no, const char *str, int cont);
...
int avl_insere(arvore_avl* avl, const char *str, int cont)
{
    if(avl == NULL || str == NULL)
        return -1;

    avl->raiz = avl_insere_impl(avl->raiz, str, cont);
...
no_avl* avl_novo_no(const char *str, int cont)
{
...
no_avl* avl_insere_impl(no_avl *no, const char *str, int cont)
{
    ...
    if (no == NULL)
        return avl_novo_no(str, cont);

    if (strcmp(str, no->str) < 0)
        no->esquerda  = avl_insere_impl(no->esquerda, str, cont);
    else if(strcmp(str, no->str) > 0)
        no->direita = avl_insere_impl(no->direita, str, cont);
    else {
        return no;
    }
