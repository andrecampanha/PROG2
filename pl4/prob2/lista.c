...
lista *lista_pesquisa_substring(lista * lst, char *substring)
{
	...
	while(el != NULL)
	{
		if (strstr(el->str, substring) != NULL)
			lista_insere(res, el->str, NULL);
		el = el->proximo;
	}
...
{
	l_elemento *el = lst2->inicio;
	
	while(el != NULL)
	{
		lista_insere(lst1, el->str, NULL);
		el = el->proximo;
	}
...
int lista_insere_ordenada(lista *lst, char *inserir)
{
	...
	while(el != NULL)
	{
		aux = strcasecmp(inserir, el->str);
		if(aux == 0) return -1;
		if(aux < 0)
		{
			lista_insere(lst, inserir, el);
			return i;
		}
		i++;
		el = el->proximo;
