...
{
    // Complexidade O(1)
    if(!g || origem < 1 || origem > g->tamanho || dest < 1 || dest > g->tamanho) return -1;

    if(g->adjacencias[origem - 1][dest - 1]) return 0;

    g->adjacencias[origem - 1][dest - 1] = 1;
    return 1;
}
...
int grafo_remove(grafo *g, int origem, int dest)
{
    ...
    g->adjacencias[origem - 1][dest - 1] = 0;
    return 1;
}
...
int grafo_aresta(grafo *g, int origem, int dest)
{
    // Complexidade O(1)
    if(!g || origem < 1 || origem > g->tamanho || dest < 1 || dest > g->tamanho) return -1;
    return g->adjacencias[origem - 1][dest - 1];
}
...
int grafo_completo(grafo* g)
{
    ...
    for(int i = 1; i <= g->tamanho; i++)
    {
        for(int j = 1; j <= g->tamanho; j++)
        {
            if(i != j && !grafo_aresta(g, i, j)) return 0;
        }
    }
    return 1;
}
...
{
    // Complexidade: O(N)
    if(!g || i < 0 || i > g->tamanho) return -1;

    for(int j = 1; j <= g->tamanho; j++)
    {
        if(i != j && (grafo_aresta(g, i, j) || !grafo_aresta(g, j, i))) return 0;
    }
...
int grafo_temCelebridade(grafo* g)
{
    ...
    for(int i = 1; i <= g->tamanho; i++)
    {
        if(grafo_eCelebridade(g, i)) return 1;
    }
